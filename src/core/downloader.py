...
def resolve_workspace_dir(output_dir: Optional[str | Path] = None) -> Path:
...
def resolve_cookie_file(cookie_file: Optional[str | Path] = None, required: bool = False) -> Optional[Path]:
...
def download_lecture(
    ...
    if is_coursera_url(url):
        return download_coursera_media(url, cookie_file=cookie_file, output_dir=output_dir,
                                       sub_lang=sub_lang, quality=quality)
    return download_with_ytdlp(url, output_dir=output_dir, cookie_file=cookie_file,
                               sub_lang=sub_lang, quality=quality)
