"""Transcript parsing and querying module for lore-engine."""
...
def search_transcript(
    ...
    """
    Search transcript for occurrences matching query string or regex.
    Returns matched items with surrounding subtitle context.
    """
    subs = parse_srt(srt_path)
    if not subs or not query:
        return []
    ...
    for i, sub in enumerate(subs):
        if pattern.search(sub["text"]):
            ...
            ctx_end = min(len(subs), i + context_lines + 1)
            context_entries = subs[ctx_start:ctx_end]
            
            matches.append({
                "match_index": sub["index"],
                "match_time": sub["start_time"],
                "match_seconds": sub["start_seconds"],
                "match_text": sub["text"],
                "context": [
                    f"[{c['start_time']}] {c['text']}" for c in context_entries
                ]
            })
