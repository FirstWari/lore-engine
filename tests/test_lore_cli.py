...
def fake_pipeline(monkeypatch, tmp_path):
    ...
    def fake_keyframes(path, output_dir=None, max_frames=25, candidate_samples=60, **kw):
        ...
        for ts in ("00-00-05", "00-10-00"):
            ...
            f.write_bytes(b"jpg")
            frames.append(
                {"timestamp": ts.replace("-", ":"), "seconds": 0, "frame_index": 0, "image_path": str(f)}
            )
        return frames
...
def test_url_is_downloaded_then_processed(fake_pipeline, tmp_path, monkeypatch):
    ...
    def fake_download(url, output_dir=None, cookie_file=None, sub_lang="en", quality="720p"):
        ...
        return {
            ...
            "srt_path": str(video.with_suffix(".srt")),
            "quality": "240p",
            "subtitle_error": None,
        }
        ...
        return [
            {
                ...
                "seconds": i,
                "frame_index": i,
                "image_path": str(Path(output_dir) / f"f{i}.jpg"),
            }
            for i in range(n)
        ]

    monkeypatch.setattr(lore, "extract_keyframes", fake_keyframes)
    frames, used = lore._select_keyframes(
        tmp_path / "v.mp4", tmp_path / "kf", 1000.0, max_frames=27, candidates=170
    )
